...
nlp = StanfordCoreNLP("/Users/tangruili/Downloads/STF/stanford-corenlp-4.2.2")
# the following two lines are necessary for some unknown reasons
s = "When did Herve serve as a Byzantine general?"
dependency = nlp.dependency_parse(s)
...
class EtN:
    ...
    def token_complex(self,semi_cpt):
        if semi_cpt.motion_complex():
            ...
            self.instant_translation.append("<"+semi_cpt.subj+"-->(/,"+semi_cpt.name+"_complex,_,"+semi_cpt.obj+")>.")
            self.instant_translation.append("<"+semi_cpt.obj+"-->(/,"+semi_cpt.name+"_complex,"+semi_cpt.subj+",_)>.")
    ...
    def advcl(self,A,B):
        # advcl(talk,know)
        # if the first verb needs a object, B is its subject
        if A.obj == "$":
            A.obj = self.token_complex(B)
        if A.subj != "$" and A.obj != "$":
            self.token_complex(A)
...
s = "I am studying Chinese diligently."
#s = "I am studying English."
#s = "What I am studying diligently?"
